...
address payable constant VAULT_IMPLEMENTATION = payable(
...
contract ForkTest is Test {
    ...
    function setUp() public {
        console.log(
            unicode"🛞 Setup Fork testing. Block number: ",
            block.number
        );
        ...
        assertNotEq(
            address(newVaultImplementation).code,
            VAULT_IMPLEMENTATION.code
        );

        // upgrade vault
        bytes memory upgradeToCalldata = abi.encodeCall(
            IProxy.upgradeTo,
        ...
        vm.prank(VAULT_AND_SLYX_PROXY_ADMIN);
        assertEq(
            IProxy(VAULT_PROXY).implementation(),
            address(newVaultImplementation)
        );
        ...
        bytes memory slyxInitializeCalldata = abi.encodeCall(
            SLYXToken.initialize,
            (SLYX_CONTRACT_OWNER, vault)
        );

        vm.startPrank(VAULT_AND_SLYX_PROXY_ADMIN);
        IProxy(SLYX_PROXY).upgradeToAndCall(
            address(sLyxTokenImplementation),
            slyxInitializeCalldata
        );
        ...
        console.log(
            unicode"🧪 Starting Fork testing. Block number: ",
            block.number
        );
    }
    ...
    function test_ExchangeRateForSLYXToLyxStartAt1To1SinceNoSLYXGotMinted()
        public
        view
    {
    ...
    function test_NewUserStakeAndMintSLYXLater() public {
        ...
        assertEq(
            ...
            aliceStake - VAULT_ROUNDING_ERROR_LOSS
        );
        assertEq(
            sLyxToken.totalSupply(),
            aliceShares - VAULT_ROUNDING_ERROR_LOSS
        );
        assertEq(
    ...
    function test_NewUserMintSLYXAtTheSameTimeThanDepositing() public {
        ...
        assertEq(
            vault.balanceOf(address(sLyxToken)),
            depositAmount - VAULT_ROUNDING_ERROR_LOSS
        );
    ...
    function test_NewUserStakeForSomeoneElseWhoMintSLYXLater() public {
        ...
        assertEq(
            vault.balanceOf(address(sLyxToken)),
            bobStake - VAULT_ROUNDING_ERROR_LOSS
        );
        assertEq(
            sLyxToken.totalSupply(),
            bobShares - VAULT_ROUNDING_ERROR_LOSS
        );
        assertEq(
            sLyxToken.balanceOf(bob),
            bobShares - VAULT_ROUNDING_ERROR_LOSS
        );
    ...
    function test_ExistingStakerConvertSomeOfItsStakeToSLYX() public {
        ...
        assertEq(
            vault.balanceOf(largeStaker),
            largeStakerStake - amountToConvertAsSLYX + VAULT_ROUNDING_ERROR_LOSS
        );
        assertEq(
            vault.balanceOf(address(sLyxToken)),
            amountToConvertAsSLYX - VAULT_ROUNDING_ERROR_LOSS
        );
        assertEq(
            vault.sharesOf(largeStaker),
            largeStakerShares / 2 + VAULT_ROUNDING_ERROR_LOSS
        );
        ...
        assertEq(
            sLyxToken.balanceOf(largeStaker),
            vault.sharesOf(largeStaker) - VAULT_ROUNDING_ERROR_LOSS
        );
    }
    ...
    function test_ExistingStakeConvertAllItsStakeToSLYX() public {
        ...
        assertEq(
            vault.balanceOf(address(sLyxToken)),
            userStake - VAULT_ROUNDING_ERROR_LOSS
        );

        assertEq(
            sLyxToken.totalSupply(),
            userShares - VAULT_ROUNDING_ERROR_LOSS
        );
        assertEq(
            sLyxToken.balanceOf(existingUser),
            userShares - VAULT_ROUNDING_ERROR_LOSS
        );
    }

    function test_ExchangeRateForSLYXToLyxStartIncreasesAfterMintingSLYX()
        public
    {
    ...
    function test_UserReceivesSLYXAndBurnThemToConvertToStake() public {
        ...
        assertEq(
            sLyxToken.balanceOf(existingUser),
            userSLYXBalance - amountToTransfer
        );
        ...
        assertEq(sLyxToken.balanceOf(alice), 0);
        uint256 expectedLYXStake = sLyxToken.getNativeTokenValue(
            amountToTransfer
        );
    ...
    function test_UserReceivesSLYXAndBurnThemToConvertToStakeAfterAccumulatingRewards()
        public
    {
        address existingUser = 0xC99Fe517d665c23035B050e13188eb4d7075C91d;
        uint256 userStake = vault.balanceOf(existingUser);
        ...
        assertEq(
            sLyxToken.balanceOf(existingUser),
            userSLYXBalance - sLyxAmountToTransfer
        );
        assertEq(sLyxToken.balanceOf(alice), sLyxAmountToTransfer);

        uint256 exchangeRateBefore = sLyxToken.getExchangeRate();
        uint256 sLyxToLYXValueBefore = sLyxToken.getNativeTokenValue(
            sLyxAmountToTransfer
        );
        ...
        uint256 sLyxToLYXValueAfter = sLyxToken.getNativeTokenValue(
            sLyxAmountToTransfer
        );
        ...
        assertEq(
            vault.balanceOf(alice),
            sLyxToLYXValueAfter - VAULT_ROUNDING_ERROR_LOSS
        );
